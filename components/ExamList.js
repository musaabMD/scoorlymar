...
//         // Fetch all active exams from the exams table
//         const { data, error } = await supabase
//           .from('exams')
//           .select(`
//             *,
//             exam_categories(name, slug, color)
//           `)
//           .eq('is_active', true)
//           .order('display_order', { ascending: true });
...
//         // Process exams to include necessary display properties
//         const processedExams = data.map(exam => {
//           // Generate a gradient based on the exam color or name
//           const gradient = getGradientForExam(exam.name, exam.color);
...
//             gradient,
//             icon: <BookOpen size={24} className="text-white" />,
//             // Use a default question count if not provided
//             questions: exam.question_count || Math.floor(Math.random() * 300) + 100
//           };
//         });
...
//     // Filter by tab if not "all"
//     if (activeTab !== 'all') {
//       filtered = filtered.filter(exam => exam.category_id === activeTab);
//     }
...
//                   // Generate the URL in the format /exam/slug
//                   const examUrl = exam.slug ? `/exam/${exam.slug}` : `/exams/${exam.id}`;
  ...
  useEffect(() => {
    async function fetchExams() {
      try {
        ...
        console.log('Fetched exams:', data); // Log data to help with debugging
        ...
        const processedExams = data.map(exam => {
          ...
          // Make sure we correctly process the exam_categories data
          const categoryData = exam.exam_categories;
          
          return {
            ...exam,
            // Make sure we have the category data in a consistent format
            exam_categories: categoryData,
            gradient,
            icon: <BookOpen size={24} className="text-white" />,
            // Use a default question count if not provided
            questions: exam.question_count || Math.floor(Math.random() * 300) + 100
        ...
        <>
          ...
          <div>
            {filteredExams.length > 0 ? (
              <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-y-6 gap-x-4">
                {filteredExams.map(exam => {
                  // Generate the URL in the format /exam/[categorySlug]/[examSlug]
                  ...
                  const examUrl = exam.slug ? `/exam/${categorySlug}/${exam.slug}` : `/exams/${exam.id}`;
