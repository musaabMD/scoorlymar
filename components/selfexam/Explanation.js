...
const Explanation = ({ rationale, isVisible, explanationImageUrl }) => {
  ...
  const processedRationale = useMemo(() => {
  ...
  // Use the processed rationale, but conditionally render the component
  if (!isVisible) return null;
  
  const { mainExplanation, incorrectAnswers } = processedRationale;
